...
import {useHome} from "@/shared/lib/Context/home-context"
...
function SubMenu({ cateGroups, id }: SubMenuProps) {
    const {hanldeNavToJobListPage } =useHome();
  ...
  return (
    <div className="grid grid-cols-2 content-start gap-2 rounded-2xl bg-white text-sm p-3 h-68 overflow-y-auto ">
          {categories.map((c)=>{
            return <p onClick={()=>hanldeNavToJobListPage(c.slug)} className="py-1 w-fit grid-cols-1 px-3 bg-gray-200 rounded-full" key={c.id}>{c.name}</p>
          })} 
    </div>
  );
}
